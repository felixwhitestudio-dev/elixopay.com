...
exports.googleOAuth = async (req, res) => {
  try {
    ...
    if (!user) {
      // Register new Google user using current schema
      ...
      try {
        await db.query(
          ...
          [user.id, uuidv4(), 0, 'THB', true]
        );
      } catch (e) { console.error('Create wallet failed', e.message); }
    ...
    try {
      await db.query('INSERT INTO sessions (user_id, refresh_token, expires_at, ip_address, user_agent) VALUES ($1,$2,$3,$4,$5)', [
        user.id, refreshHash, expiresAt.toISOString(), req.ip, req.get('user-agent') || ''
      ]);
